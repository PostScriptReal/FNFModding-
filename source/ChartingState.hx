...
class ChartingState extends MusicBeatState
{
	...
	var dropDownMenu:Bool = false;
	var isAltNoteCheck:FlxUICheckBox;
	var player1TextField:FlxUIDropDownMenuCustom;
	var characters:Array<String>;
	var stages:Array<String>;
	var cutscene:Array<String>;
	var ui:Array<String>;
	...
	**/
	...
	function addSongUI():Void
	{
		...
		// var charList:Array<String> = CoolUtil.coolTextFile('assets/images/custom_chars/charList.txt');
		var charJson:Dynamic = CoolUtil.parseJson(FNFAssets.getJson('assets/images/custom_chars/custom_chars'));
		var stageList:Array<String> = CoolUtil.coolTextFile('assets/images/custom_stages/stages.txt');
		var cutsceneList:Array<String> = CoolUtil.coolTextFile('assets/images/custom_cutscenes/cutscenes.txt');
		var uiList:Array<String> = CoolUtil.coolTextFile('assets/images/custom_ui/ui_packs/uitypes.txt');

		var characters = Reflect.fields(charJson);

        trace(characters);
        var stringy:String = ''+ characters;
        trace("Comparison: "+ stringy);
        // trace("What this should look like: "+charList);

		var player1TextField = new FlxUIDropDownMenuCustom(9, 95, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(char:String)
		{
        	char = ''+ characters;
			_song.player1 = characters[Std.parseInt(char)];
			trace("What will be displayed: "+ _song.player1);
		});
		player1TextField.selectedLabel = _song.player1;
		blockPressWhileScrolling.push(player1TextField);
		var player2TextField = new FlxUIDropDownMenuCustom(149, 95, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(char:String)
		{
        	char = ''+ characters;
			_song.player2 = characters[Std.parseInt(char)];
		});
		player2TextField.selectedLabel = _song.player2;
		blockPressWhileScrolling.push(player2TextField);
		var gfTextField = new FlxUIDropDownMenuCustom(9, 130, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(char:String)
		{
        	char = ''+ characters;
			_song.gf = characters[Std.parseInt(char)];
		});
		gfTextField.selectedLabel = _song.gf;
		blockPressWhileScrolling.push(gfTextField);
		var stageTextField = new FlxUIDropDownMenuCustom(149, 130, FlxUIDropDownMenuCustom.makeStrIdLabelArray(stageList, true), function(stage:String)
		{
			var stageJson:Dynamic = null;
			stageJson = CoolUtil.parseJson(FNFAssets.getJson('assets/images/custom_stages/custom_stages'));
			if (stages == null) {
            	stages = Reflect.fields(stageJson);
        	}
			_song.stage = stageJson[Std.parseInt(stage)];
		});
		stageTextField.selectedLabel = _song.stage;
		blockPressWhileScrolling.push(stageTextField);
		var cutsceneTextField = new FlxUIDropDownMenuCustom(149, 165, FlxUIDropDownMenuCustom.makeStrIdLabelArray(cutsceneList, true), function(cutscenes:String)
		{
			var cutsceneJson:Dynamic = null;
			cutsceneJson = CoolUtil.parseJson(FNFAssets.getJson('assets/images/custom_cutscenes/custom_cutscenes'));
			if (cutscene == null) {
            	cutscene = Reflect.fields(cutsceneJson);
        	}
			_song.cutsceneType = cutsceneJson[Std.parseInt(cutscenes)];
		});
		cutsceneTextField.selectedLabel = _song.cutsceneType;
		blockPressWhileScrolling.push(cutsceneTextField);
		var uiTextField = new FlxUIDropDownMenuCustom(9, 165, FlxUIDropDownMenuCustom.makeStrIdLabelArray(uiList, true), function(uiTypes:String)
		{
			var uiJson:Dynamic = null;
			uiJson = CoolUtil.parseJson(FNFAssets.getJson('assets/images/custom_cutscenes/custom_cutscenes'));
			if (ui == null) {
            	ui = Reflect.fields(uiJson);
        	}
			_song.uiType = uiJson[Std.parseInt(uiTypes)];
		});
		uiTextField.selectedLabel = _song.uiType;
		blockPressWhileScrolling.push(uiTextField);
	...
	function addCharsUI():Void
	{
		...
		var player1TextField = new FlxUIDropDownMenuCustom(9, 95, FlxUIDropDownMenuCustom.makeStrIdLabelArray(charList, true), function(char:String)
		{
			_song.player1 = charList[Std.parseInt(char)];
		});
		player1TextField.selectedLabel = _song.player1;
		blockPressWhileScrolling.push(player1TextField);
		var player2TextField = new FlxUIDropDownMenuCustom(149, 95, FlxUIDropDownMenuCustom.makeStrIdLabelArray(charList, true), function(char:String)
		{
			_song.player2 = charList[Std.parseInt(char)];
		});
		player2TextField.selectedLabel = _song.player2;
		blockPressWhileScrolling.push(player2TextField);
		var gfTextField = new FlxUIDropDownMenuCustom(9, 130, FlxUIDropDownMenuCustom.makeStrIdLabelArray(charList, true), function(char:String)
		{
			_song.gf = charList[Std.parseInt(char)];
		});
		gfTextField.selectedLabel = _song.gf;
		blockPressWhileScrolling.push(gfTextField);
		var stageTextField = new FlxUIDropDownMenuCustom(149, 130, FlxUIDropDownMenuCustom.makeStrIdLabelArray(stageList, true), function(stage:String)
		...
		var cutsceneTextField = new FlxUIDropDownMenuCustom(149, 165, FlxUIDropDownMenuCustom.makeStrIdLabelArray(cutsceneList, true), function(cutscenes:String)
		{
			_song.cutsceneType = cutsceneList[Std.parseInt(cutscenes)];
		});
		cutsceneTextField.selectedLabel = _song.cutsceneType;
		blockPressWhileScrolling.push(cutsceneTextField);
		var uiTextField = new FlxUIDropDownMenuCustom(9, 165, FlxUIDropDownMenuCustom.makeStrIdLabelArray(uiList, true), function(uiTypes:String)
		{
			_song.uiType = uiList[Std.parseInt(uiTypes)];
		});
		uiTextField.selectedLabel = _song.uiType;
		blockPressWhileScrolling.push(uiTextField);
		playerText = new FlxText(player1TextField.x, player1TextField.y - 15, 0, "Player", 8, false);
		enemyText = new FlxText(player2TextField.x, player2TextField.y - 15, 0, "Enemy", 8, false);
		gfText = new FlxText(gfTextField.x, gfTextField.y - 15, 0, "GF", 8, false);
		stageText = new FlxText(stageTextField.x, stageTextField.y - 15, 0, "Stage", 8, false);
		cutsceneText = new FlxText(cutsceneTextField.x, uiTextField.y - 15, 0, "Cutscene", 8, false);
		uiText = new FlxText(uiTextField.x, uiTextField.y - 15, 0, "UI", 8, false);
	...
	function changeKeyType(change:Int) {
		...
		switch (noteType)
		{
			...
			case Danger:
				noteTypeText.text = "Danger Note";
			case 5:
				// drain
				noteTypeText.text = "Drain Note";
			default:
				noteTypeText.text = 'Custom Note ${noteType - 5}';
		}
	}
	function loadSong(daSong:String):Void
	{
		...
		#if sys
		if (FNFAssets.exists("assets/songs/" + daSong + "/" + "Inst" + TitleState.soundExt)) {
			FlxG.sound.playMusic(Sound.fromFile("assets/songs/" + daSong + "/" + "Inst"+TitleState.soundExt), 0.6);
		} else {
		...
		var vocalSound = Sound.fromFile("");
		if (_song.needsVoices) {
			#if sys
			if (FNFAssets.exists("assets/songs/" + daSong + "/" + "Voices" + TitleState.soundExt)) {
				vocalSound = Sound.fromFile("assets/songs/" + daSong + "/" + "Voices"+TitleState.soundExt);
			} else {
			...
			#else
			if (FNFAssets.exists("assets/songs/" + daSong + "/" + "Voices" + TitleState.soundExt)) {
				vocals = new FlxSound().loadEmbedded("assets/songs/" + daSong + "/" + "Voices" + TitleState.soundExt);
			} else {
	...
	override function update(elapsed:Float)
	{
		...
		for (typingShit in blockPressWhileTypingOn) {
