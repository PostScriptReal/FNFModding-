...
import NoteOptionsState.HSBJson;
#if FEATURE_LUAMODCHART
typedef LuaAnim = {
	var prefix : String;
...
{
	public static var instance:PlayState = null; // KADE LUA ONLY 
	#if windows
	public static var customPrecence = FNFAssets.getText("assets/discord/presence/play.txt");
	#end
	public static var curStage:String = '';
	#if FEATURE_LUAMODCHART
	public static var LuaStage:KadeStage; // KADE LUA ONLY
	#end
	...
	public static var kadeDad:Character;
	...
	public var kadeAccuracy:Float = 0.00;
	private var accuracyDefault:Float = 0.00;
	public static var sicks:Int = 0;
	public static var hows:Int = 0;
	...
	public static var kadeStrumLineNotes:FlxTypedGroup<KadeArrow> = null; // KADE LUA ONLY
	var isKadeLua:Bool = false;
	public static var kadePlayerStrums:FlxTypedGroup<KadeArrow> = null; // KADE LUA ONLY
	public static var kadeCpuStrums:FlxTypedGroup<KadeArrow> = null; // KADE LUA ONLY
	private var strumming2:Array<Bool> = [false, false, false, false];
	private var strumming1:Array<Bool> = [false,false,false,false];
	var noMoreEnemyStrums = false;
	var noMorePlayerStrums = false;
	var dontStartSong:Bool = false;
	...
	public var uiSmelly:TUI;
	public var uiSussy:TUI;
	public var sploosh:NoteSplash;
	var pixelShitPart2:String = '';
	public static var splashusStringus:String = '';
	#if (windows && cpp)
	...
	public var lyricsCam:FlxCamera;
	public var strumCammy:FlxCamera;
	private var camGame:FlxCamera;
	public var camNotes:FlxCamera; // KADE LUA ONLY
	public var camSustains:FlxCamera; // KADE LUA ONLY

	public var unhookStrumsFromHUD:Bool = false;
	public var unhookLyricsFromHUD:Bool = true;
	public var unnamedvariable:Bool = false;

	public var doof:DialogueBox;
	public var daRating:String = "sick";
	// public var daVideo:String = 'ughCutscene.mp4';
	...
	var ostLogo:FlxSprite;
	var blackk:FlxSprite;
	...
	// public static var waitHow:Bool = false;
	...
	var notpixel:Bool = false;
	var splashopt:Bool = false;
	var pixelSplash:Bool = false;
	...
	private var antioptList:Array<String> = [];
	/**
	...
	public static var keepSelectedBf:Bool;
	public static var keepSelectedDad:Bool;
	public static var bfString:String;
	public static var dadString:String;
	public static var cutsceneSkip:Bool = false;
	// Intentionally putting in a typo here...
	static var resverse:Bool = false;
	var colourSwap:ColourSwap;
	var blankShader:ColourSwap;
	var reflectList:Array<String> = ['purple', 'blue', 'green', 'red'];
	...
	function getHaxeActor(name:String):Dynamic {
		switch (name) {
			...
			case "dad":
				return dad;
			/*case "strumLineNotes" | "strumlinenotes" | "strumLine" | "strumline":
				return strumLineNotes;*/
			default:
				// if (!isKadeLua)
				return strumLineNotes.members[Std.parseInt(name)];
				/*else
					return kadeStrumLineNotes.members[Std.parseInt(name)];*/
		}
	}
	public static function flushData() {
		splashusStringus = '';
		keepSelectedBf = false;
		keepSelectedDad = false;
		bfString = '';
		dadString = '';
	}
	public static function cloneHero() {
		resverse = true;
		FlxG.sound.music.pause;
		vocals.pause;
		FlxG.sound.music.volume = 0;
		vocals.volume = 0;
		FlxG.sound.play('assets/sounds/cloneherosoundlol.ogg', 1, false, null, true, function() {
			resverse = false;
	...
	function makeHaxeState(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("camSTRUMS", strumCammy);
		interp.variables.set("unhookFromHUD", function (hookers:String) {
			switch(hookers) {
				case 'strum':
					unhookStrumsFromHUD = true;
					strumLineNotes.cameras = [strumCammy];
					grpNoteSplashes.cameras = [strumCammy];
					notes.cameras = [strumCammy];
					trace("Unhooked Strums");
				case 'strums':
					unhookStrumsFromHUD = true;
					strumLineNotes.cameras = [strumCammy];
					grpNoteSplashes.cameras = [strumCammy];
		...
		interp.variables.set("rehookToHUD", function (hookers:String) {
			switch(hookers) {
				case 'strum':
					unhookStrumsFromHUD = false;
					strumLineNotes.cameras = [camHUD];
					grpNoteSplashes.cameras = [camHUD];
					notes.cameras = [camHUD];
					trace("Rehooked Strums");
				case 'strums':
					unhookStrumsFromHUD = false;
					strumLineNotes.cameras = [camHUD];
					grpNoteSplashes.cameras = [camHUD];
		...
		// idk why, but doing this doesn't work
		// interp.variables.set("Math", Math);
		interp.variables.set("Math.sin", Math.sin);
		interp.variables.set("Math.cos", Math.cos);
		interp.variables.set("Math.pi", Math.PI);
		...
		interp.variables.set("showOnlyPlayerStrums", false);
		interp.variables.set("dontStartSong", false);
		...
		interp.variables.set("vcr", OptionsHandler.options.vhs);
		...
		interp.variables.set("swapUI", function (uiStupid:String) {
			// go crazy go stupid
			switch(uiStupid) {
				default:
					uiSussy = Reflect.field(Judgement.uiJson, uiStupid);
					uiSmelly = uiSussy;
					pixelUI = uiSmelly.isPixel;
					splashusStringus = uiStupid;
					if (uiSmelly.isPixel) {
						pixelShitPart2 = '-pixel';
					}
					var splooshus:NoteSplash;
					grpNoteSplashes.forEach(function(sploosh:NoteSplash)
					{
						grpNoteSplashes.remove(sploosh);
						sploosh.destroy();
					});
					splooshus = new NoteSplash(100, 100, 0);
					grpNoteSplashes.add(splooshus);
					SONG.uiType = uiStupid;
					killBabyArrows();
					/*var daStrumTime:Float = songNotes[0] + OptionsHandler.options.offset;
					notes.forEachAlive(function(daNote:Note) {
						daNote = new Note(daStrumTime, songNotes[1], oldNote, false, customImage, customXml, arrowEndsImage, daLift, animSuffix);
					});
					notes.forEachAlive(function(tokill:Note)
					{
						notes.remove(tokill);
						tokill.destroy();
					});*/
					...
					// generateSong(SONG.song, true);
					// notes.cameras = [camHUD];
			}
		});
		interp.variables.set("startFunction", function (daFunction:String, ?arguments:Array<Dynamic>) {
/*			var argument:String = "";
			if (arguments == null) {
				argument = 'modchart';
			}*/
			switch(daFunction) {
				...
				default:
					if (arguments == null) {
						callHscript(daFunction, [], usehaxe);
					} else {
						callHscript(daFunction, [arguments], usehaxe);
					}
			}
		});
		interp.variables.set("strumLine", function (manipulate:String, valuee:Int, notess:Bool) {
			return moveStrumBabies(manipulate, valuee, notess);
		});
		interp.variables.set("setStrumPos", function (valuee:Float, valuee2:Float, addToX:Bool, notess:Bool) {
			return setStrumPos(valuee, valuee2, addToX, notess);
		});
		interp.variables.set("executeNewModchart", function (file:String) {
			makeHaxeState("modchart", "assets/data/" + SONG.song.toLowerCase() + "/", file);
		});
		interp.variables.set("songPosition", Conductor.songPosition);
		trace("set stuff");
	...
	function makeLyricState(usehaxe:String, path:String, filename:String) 
	{
		trace("Lyrics Time!!");
		var parser = new ParserEx();
		var program = parser.parseString(FNFAssets.getHscript(path + filename));
		var interp = PluginManager.createSimpleInterp();
		interp.variables.set("camLYRICS", lyricsCam);
		interp.variables.set("lyrics", omglyricslol);
		interp.variables.set("setLyrics", function (to:String, stepsuntildie:Int) {
			if (omglyricslol.text != to) {
				omglyricslol.text = to;
				trace("changed lyrics to '" + to + "'");
			}
			if (to == null) {
				omglyricslol.text = "Error: string is null, make sure you've entered a string";
			}
			if (StringTools.contains(to, "Popularsonic") || StringTools.contains(to, "PS") || StringTools.contains(to, "PCsonic") || StringTools.contains(to, "PCsonic9872") || StringTools.contains(to, "Popularsonic9872")) {
				omglyricslol.text = "Hello";
				if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
					omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.BLUE, RIGHT, OUTLINE, FlxColor.BLACK);
				} else {
					omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.BLUE, RIGHT, OUTLINE, FlxColor.BLACK);
				}
				trace("Funni Easter Egg Activated :))))))))");
			}
			if (curStep == stepsuntildie) {
				omglyricslol.text = "";
				trace("successfully cleared lyrics");
			}
			if (omglyricslol.text != to) {
				trace("done!!!1!");
			}
		});
		interp.variables.set("lyricsShakeEffect", function (intensity:Float, duration:Float) {
			trace("Shake intensity: " + intensity + ", Shake duration: " + duration);
			lyricsCam.shake(intensity, duration);
			trace("Lyrics are having a stroke lol");
		});
		interp.variables.set("setLyricsColor", function (colour:String) {
			switch(colour) {
				case 'white':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'black':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.BLACK, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.BLACK, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'blue':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.BLUE, RIGHT, OUTLINE, FlxColor.BLACK);
				...
				case 'brown':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.BROWN, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.BROWN , RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'cyan':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.CYAN, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.CYAN, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'grey' | 'gray':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.GRAY, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.GRAY, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'green':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.GREEN, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.GREEN, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'lime':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.LIME, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.LIME, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'magenta':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.MAGENTA, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.MAGENTA, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'orange':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.ORANGE, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.ORANGE, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'pink':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.PINK, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.PINK, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'purple':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.PURPLE, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.PURPLE, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'red':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.RED, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.RED, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				case 'yellow':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.YELLOW, RIGHT, OUTLINE, FlxColor.BLACK);
					} else {
						omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.YELLOW, RIGHT, OUTLINE, FlxColor.BLACK);
					}
				default:
					trace("Error: Get better spelling skills idiot!");
			}
			trace("Changed lyrics colour to " + colour);
		});
		interp.variables.set("setLyricsColour", function (colour:String) {
			switch(colour) {
				case 'white':
					if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
						omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		...
		interp.variables.set("setLyricsRGBColour", function (r:Int, g:Int, b:Int) {
			if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
				omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.fromRGB(r, g, b, 255), RIGHT, OUTLINE, FlxColor.BLACK);
			} else {
				omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.fromRGB(r, g, b, 255), RIGHT, OUTLINE, FlxColor.BLACK);
			}
			trace("Red: " + r + " Green: " + g + " Blue: " + b);
		});
		interp.variables.set("setLyricsRGBColor", function (r:Int, g:Int, b:Int) {
			if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
				omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.fromRGB(r, g, b, 255), RIGHT, OUTLINE, FlxColor.BLACK);
			} else {
				omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.fromRGB(r, g, b, 255), RIGHT, OUTLINE, FlxColor.BLACK);
		...
		interp.variables.set("setLyricsHexColour", function (hex:String) {
			if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
				omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.fromString(hex), RIGHT, OUTLINE, FlxColor.BLACK);
			} else {
				omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.fromString(hex), RIGHT, OUTLINE, FlxColor.BLACK);
			}
			trace("colour:"+hex);
		});
		interp.variables.set("setLyricsHexColor", function (hex:String) {
			if (FNFAssets.exists('assets/fonts/lyrics.ttf')) {
				omglyricslol.setFormat("assets/fonts/lyrics.ttf", 18, FlxColor.fromString(hex), RIGHT, OUTLINE, FlxColor.BLACK);
			} else {
				omglyricslol.setFormat("assets/fonts/vcr.ttf", 18, FlxColor.fromString(hex), RIGHT, OUTLINE, FlxColor.BLACK);
	...
	/*function makeLyricState(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("lyrics", omglyricslol.text);
		interp.variables.set("start", function (song) {});
		interp.variables.set("beatHit", function (beat) {});
		interp.variables.set("update", function (elapsed) {});
		interp.variables.set("stepHit", function(step) {});
	...
	override public function create()
	{
		...
		#end
		blankShader = new ColourSwap();
		blankShader.hue = 0;
		blankShader.saturation = 0;
		blankShader.brightness = 0;
		Note.getFrames = true;
		...
		*/
		...
		executeModchart = FNFAssets.exists(Paths.luakade('songs/${SONG.song.toLowerCase()}/modchart'));
		// No SM support is planned and it won't be added.
		/*if (isSM)
			executeModchart = FileSystem.exists(pathToSm + "/modchart.lua");*/
		// Graphics options doesn't have an option for optimizing the game yet.
		/*if (executeModchart)
		...
		lyricsCam = new FlxCamera();
		lyricsCam.bgColor.alpha = 0;
		if (OptionsHandler.options.antialiasing != Everything) {
			lyricsCam.antialiasing = false;
		}
		strumCammy = new FlxCamera();
		strumCammy.bgColor.alpha = 0;
		if (OptionsHandler.options.antialiasing != Everything) {
			strumCammy.antialiasing = false;
		}
		// camSustains and camNotes are only used for lua modcharting. Weird, right?
		camSustains = new FlxCamera();
		camSustains.bgColor.alpha = 0;
		if (OptionsHandler.options.antialiasing != Everything) {
			camSustains.antialiasing = false;
		}
		camNotes = new FlxCamera();
		camNotes.bgColor.alpha = 0;
		if (OptionsHandler.options.antialiasing != Everything) {
			camNotes.antialiasing = false;
		}
		FlxG.cameras.reset(camGame);
		FlxG.cameras.add(strumCammy);
		FlxG.cameras.add(camHUD);
		FlxG.cameras.add(lyricsCam);
		...
		splashopt = OptionsHandler.options.togglesplash;
		...
		if (executeModchart)
		{
			// Not making a haxe state because of compatibility reasons as KE does modcharting differently.
			luaModchart = KadeLuaState.createModchartState(isStoryMode);
		...
		if (executeModchart)
		{
			new LuaCamera(camGame, "camGame").Register(KadeLuaState.lua);
			new LuaCamera(camHUD, "camHUD").Register(KadeLuaState.lua);
			new LuaCamera(camSustains, "camSustains").Register(KadeLuaState.lua);
			new LuaCamera(camSustains, "camNotes").Register(KadeLuaState.lua);
			new LuaCharacter(kadeDad, "dad").Register(KadeLuaState.lua);
			new LuaCharacter(gf, "gf").Register(KadeLuaState.lua);
			new LuaCharacter(boyfriend, "boyfriend").Register(KadeLuaState.lua);
		}
		#end
		...
		#if desktop
		if (!OptionsHandler.options.toaster) {
		...
		if (!keepSelectedDad) {
			...
			dad = new Character(100, 100, dadString);
		}
		if (OptionsHandler.options.antialiasing == None) {
		...
		if (!keepSelectedBf) {
			...
			boyfriend = new Character(770, 450, bfString, true);
		}
		if (OptionsHandler.options.antialiasing == None) {
		...
		if (OptionsHandler.options.toaster) {
			blackk = new FlxSprite(-200, -200).makeGraphic(2*FlxG.width,2*FlxG.height, -16777216);
		    blackk.scrollFactor.set();
		    add(blackk);
		    ostLogo = new FlxSprite(160, 25);
		    var ostPic = FNFAssets.getBitmapData("assets/images/ostLogo.png");
		    var ostXml = FNFAssets.getText("assets/images/ostLogo.xml");
		    var ostTex = FlxAtlasFrames.fromSparrow(ostPic, ostXml);
		    ostLogo.frames = ostTex;
		    ostLogo.animation.addByPrefix('idle', 'logo bumpin', 24, true);
		    ostLogo.animation.play('idle');
		    ostLogo.scrollFactor.set;
		    if (OptionsHandler.options.antialiasing != Everything) {
		    	ostLogo.antialiasing = false;
		    } else {
		    	ostLogo.antialiasing = true;
		    }
		    ostLogo.setGraphicSize(Std.int(ostLogo.width * 1.4));
		    add(ostLogo);
		    blackk.cameras = [camHUD];
			ostLogo.cameras = [camHUD];
		}
		...
		add(enemyComboBreak);
		#if FEATURE_LUAMODCHART
		if (FNFAssets.exists("assets/data/" + SONG.song.toLowerCase() + "/modchart.lua"))
		{
			kadeStrumLineNotes = new FlxTypedGroup<StaticArrow>();
			add(kadeStrumLineNotes);
			isKadeLua = true;
		}
		#end
		if (!isKadeLua) {
		...
		trace("difficultyText pt. 1");
		difficTxt = new FlxText(10, FlxG.height, 0, "", 150);
		trace("lyrics pt. 1");
		omglyricslol = new FlxText(69, FlxG.height - 120, 0, "", 250);
		
		trace("difficultyText pt. 2");
		...
		omglyricslol.scrollFactor.set();
		omglyricslol.y -= omglyricslol.height;
		if (downscroll) {
			omglyricslol.y = 120;
		}
		// omglyricslol.text = 'Amongus Sus Amongus';
		// screwy way of getting text
		trace("difficultyText pt. 3");
		difficTxt.text = DifficultyIcons.changeDifficultyFreeplay(storyDifficulty, 0).text + ' - M++ ${MainMenuState.version}';
		trace("P1 Icon");
		if (!keepSelectedBf) {
			iconP1 = new HealthIcon(SONG.player1, true);
		} else {
			iconP1 = new HealthIcon(bfString, true);
		}
		...
		if (!keepSelectedDad) {
			...
			iconP2 = new HealthIcon(dadString, false);
		}
		iconP2.y = healthBar.y - (iconP2.height / 2);
		add(iconP2);
		trace("practiceDieIcon");
		...
		if (!isKadeLua) {
			...
			kadeStrumLineNotes.cameras = [camHUD];
		}
		if (!unhookStrumsFromHUD) {
			notes.cameras = [camHUD];
		}
		...
		omglyricslol.cameras = [lyricsCam];
		practiceDieIcon.visible = false;
		if (OptionsHandler.options.antialiasing != Everything) {
			practiceDieIcon.antialiasing = false;
			sploosh.antialiasing = false;
			...
			accuracyTxt.antialiasing = false;
			difficTxt.antialiasing = false;
			omglyricslol.antialiasing = false;
		}

		add(scoreTxt);
		add(difficTxt);
		if (OptionsHandler.options.lyrics) {
			add(omglyricslol);
		}
		...
		if (OptionsHandler.options.antialiasing == Characters || OptionsHandler.options.antialiasing == None) {
			// bruh I can't do stage whyyyyyy
		}
	...
	/*function ughCutscene(name:String):Void {
		var foundFile:Bool = false;
		var fileName:String = Paths.videoFolder(name + '.' + Paths.VIDEO_EXT);
		#if sys
		...
		if (foundFile) {
			...
			(new FlxVideo(fileName)).finishCallback = function() {
				...
				startCountdown();
				trace('Video ended successfully');
			}
			trace('Task failed successfully');
		} else {
	...
	function videoIntro():Void {
		if (!cutsceneSkip) {
			inCutscene = true;
			var videoJson = CoolUtil.parseJson(FNFAssets.getText('assets/images/custom_cutscenes/videos.json'));
			if (!Reflect.hasField(videoJson, SONG.song.toLowerCase())) {
				startCountdown();
				cutsceneSkip = false;
			} else {
				var video:MP4Handler = new MP4Handler();
				FlxG.sound.music.stop();
				var daVideo:String = Reflect.field(videoJson, SONG.song.toLowerCase());
				var idInt:Int = 4;
				var naughtyID:String = '.mp4';
				if (StringTools.endsWith(daVideo, '.webm')) {
					idInt = 5;
					naughtyID = '.webm';
				}
				var naughtyVideo:String = daVideo.substring(0, daVideo.length - idInt);
				// var fileName:String = 'assets/videos/' + daVideo;
				/*var fileName:String = Paths.videoFolder(name + '.' + Paths.VIDEO_EXT);
				trace(fileJson);
				trace(fileName);
				trace(name);*/
				...
				video.finishCallback = function()
				{
					...
					// that was soooooo unneccessary
					remove(b);
					startCountdown();
				}
				if (FNFAssets.exists(Paths.video(naughtyVideo +'-naughty'+ naughtyID)) && OptionsHandler.options.hahanaughty) {
					daVideo = naughtyVideo + '-naughty' + naughtyID;
				}
				trace(Paths.video(daVideo));
				video.playVideo(Paths.video(daVideo));
			}
		} else {
			startCountdown();
			cutsceneSkip = false;
	...
	#if FEATURE_LUAMODCHART
	public static var luaModchart:KadeLuaState = null;
	#end
	...
	public function startCountdown():Void
	{
		if (!dontStartSong) {
			...
			if (FNFAssets.exists("assets/data/" + SONG.song.toLowerCase() + "/lyrics", Hscript))
			{
				makeLyricState("lyrics", "assets/data/" + SONG.song.toLowerCase() + "/", "lyrics");
	...
	function startSong():Void
	{
		if (!dontStartSong) {
			...
			var useSong = "";
			if (FNFAssets.exists("assets/songs/" + SONG.song + "/" + "Inst" + TitleState.soundExt)) {
				useSong = "assets/songs/" + SONG.song + "/" + "Inst" + TitleState.soundExt;
			} else {
				useSong = "assets/music/" + SONG.song + "_Inst" + TitleState.soundExt;
			}
			if (OptionsHandler.options.stressTankmen && FNFAssets.exists("assets/songs/" + SONG.song + "/" + "Shit_Inst.ogg"))
				useSong = "assets/songs/" + SONG.song + "/" + "Shit_Inst.ogg";
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var useSong = "";
		if (FNFAssets.exists("assets/songs/" + SONG.song + "/" + "Voices" + TitleState.soundExt)) {
			useSong = "assets/songs/" + SONG.song + "/" + "Voices" + TitleState.soundExt;
		} else {
			useSong = "assets/music/" + SONG.song + "_Voices" + TitleState.soundExt;
		}
		if (OptionsHandler.options.stressTankmen && FNFAssets.exists("assets/songs/" + SONG.song + "/" + "Shit_Voices.ogg"))
			useSong = "assets/songs/" + SONG.song + "/" + "Shit_Voices.ogg";
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				*/
				...
				#if FEATURE_LUAMODCHART
				var swagNote:Note = new Note(daStrumTime, songNotes[1], oldNote, false, customImage, customXml, arrowEndsImage, daLift, animSuffix, null, songNotes[4]);
				#else
				var swagNote:Note = new Note(daStrumTime, songNotes[1], oldNote, false, customImage, customXml, arrowEndsImage, daLift, animSuffix);
				#end
				if (!swagNote.dontEdit && !swagNote.mineNote && !swagNote.nukeNote && !swagNote.dangerNote && !swagNote.isLiftNote) {
	...
	public function killBabyArrows():Void
	{
		playerStrums.forEach(function(tokill:FlxSprite)
		{
			playerStrums.remove(tokill);
			tokill.destroy();
		});
		enemyStrums.forEach(function(tokill:FlxSprite)
		{
			enemyStrums.remove(tokill);
			tokill.destroy();
		});
		strumLineNotes.forEach(function(tokill:FlxSprite)
		{
			strumLineNotes.remove(tokill);
			tokill.destroy();
		});
	}
	public function moveStrumBabies(cmd:String, value:Float, note:Bool):Void
	{
		switch (cmd) {
			case 'x' | 'X':
	...
	public function setStrumPos(value:Float, value2:Float, addToCurPos:Bool, note:Bool):Void
	{
		strumLineNotes.forEach(function(spr:FlxSprite)
		{
			if (!addToCurPos) {
		...
		if (note) {
			notes.forEachAlive(function (daNote) {
				if (!addToCurPos) {
					...
					daNote.y == daNote.y + value2;
				}
			});
		}
	}

	private function generateStaticArrows(player:Int, regenerateCheck:Bool):Void
	{
		for (i in 0...4)
		{
			// FlxG.log.add(i);
			...
			if (!isStoryMode && !regenerateCheck)
			{
				babyArrow.y -= 10;
				babyArrow.alpha = 0;
			...
			var parsedShader:HSBJson = Reflect.field(shaderJSON, reflectList[i]);
			...
			shaderList.push(colourSwap);
			shaderData.push(parsedShader);
			babyArrow.shader = shaderList[i].shader;
			babyArrow.x += 50;
			...
			if (!duoMode) {
				if (middlescroll) {
					if (!opponentPlayer) {
						babyArrow.x -= 275;
						// Now why didn't I just use playerStrums or enemyStrums??
						// Uhhhhhhhhhhhhh... I forgor.
						if (player == 0) {
			...
			if (!isKadeLua) {
				...
				kadeStrumLineNotes.add(babyArrow);
			#end
			}
			if (player == 1) {
	...
	function comboBreak(dir:Int, playerOne:Bool = true, rating:String = 'miss') {
		...
		if (unhookStrumsFromHUD && strumCammy.angle != 0) {
			thingToDisplay.angle = strumCammy.angle;
		}
		if (unhookStrumsFromHUD && strumCammy.y != 0) {
			thingToDisplay.y = strumCammy.y;
		}
		if (unhookStrumsFromHUD && strumCammy.x != 0) {
			thingToDisplay.x = strumCammy.x;
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (unhookLyricsFromHUD = false) {
			lyricsCam.x = camHUD.x;
			lyricsCam.y = camHUD.y;
			lyricsCam.angle = camHUD.angle;
		}
		lyricsCam.zoom = camHUD.zoom;
		if (unhookStrumsFromHUD = false) {
			strumCammy.x = camHUD.x;
			strumCammy.y = camHUD.y;
			strumCammy.angle = camHUD.angle;
		}
		strumCammy.zoom = camHUD.zoom;
		...
		if (hscriptStates.exists("modchart")) {
			...
			if (getHaxeVar("showOnlyPlayerStrums", "modchart"))
			{
				healthBarBG.visible = false;
				healthBar.visible = false;
				iconP1.visible = false;
				...
				scoreTxt.visible = false;
				if (!middlescroll) {
					noMoreEnemyStrums = true;
				}
				if (noMoreEnemyStrums) {
					if (!opponentPlayer) {
						enemyStrums.forEach(function(daNote:FlxSprite)
						{
							daNote.visible = false;
						});
					} else {
						playerStrums.forEach(function(daNote:FlxSprite)
						{
							daNote.visible = false;
						});
					}
		...
		if (snapToStrumline) {
			if (!middlescroll) {
				...
				if (!isKadeLua)
					...
					daNote.x = kadeStrumLineNotes.members[noteData].x;
		...
		if (perfectMode && !Ratings.CalculateFullCombo(How))
		{
			if (opponentPlayer)
				health = 50;
			else
					...
					else
					{
						if (!resverse) {
							if (daNote.mustPress)
								...
								/*else
									...
									daNote.y = (kadeStrumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y
										- 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(FlxG.save.data.scrollSpeed == 1 ? SONG.speed : FlxG.save.data.scrollSpeed,
											2));
								}*/
						} else {
						...
						if (daNote.isSustainNote)
						{
								...
								&& (!daNote.mustPress || (daNote.wasGoodHit || (daNote.prevNote.wasGoodHit && !daNote.canBeHit))))
							{
								...
								daNote.clipRect = swagRect;
								/*} else {
									swagRect.y = (kadeStrumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y
										+ Note.swagWidth / 2
										- daNote.y) / daNote.scale.y;
									swagRect.height -= swagRect.y;
				...
				if (executeModchart && luaModchart != null && songStarted)
				{
					...
					for (i in 0...4)
					{
						if (!isKadeLua)
							...
							kadeStrumLineNotes.members[i].visible = p1;
	...
	private function popUpScore(strumtime:Float, daNote:Note, playerOne:Bool, forceMiss:Bool = false):Void
	{
		...
		if (!daNote.mineNote) {
			...
			{
				case 'shit':
					if (OptionsHandler.options.playMissAnim)
					{
		...
		if (daNote.nukeNote && daRating != 'miss')
			// die <3
			healthBonus = -4;
		if (daNote.dangerNote && daRating == 'miss') {
			healthBonus = -4;
		}
		...
		if (msTiming == 0 && !FlxG.save.data.botplay) {
			// waitHow = true;
		}
		/*if (msTiming == 0 && !FlxG.save.data.botplay && waitHow) {
			waitHow = false;
		}*/
		...
		if (!note.wasGoodHit)
		{
			...
			*/
			if (note.shouldBeSung) {
				...
				var hitSoundList = CoolUtil.coolTextFile('assets/data/hitSounds.txt');
				var daHitSound:String = hitSoundList[OptionsHandler.options.hitSounds];
				if (daHitSound == 'default') {
					FlxG.sound.play(FNFAssets.getSound("assets/sounds/hitSound.ogg"));
				} else if (daHitSound != 'none' && daHitSound != 'default') {
					FlxG.sound.play(FNFAssets.getSound("assets/sounds/"+ daHitSound +"hitSound.ogg"));
				} else if (daHitSound == 'none') {
					// no more hitsounds
				}
				if (playerOne) {
					#if FEATURE_LUAMODCHART
