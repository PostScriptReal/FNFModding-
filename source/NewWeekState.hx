...
class NewWeekState extends MusicBeatState
{
	...
	override function create()
	{
		if (!sorted) {
			...
			flshText = new FlxUIInputText(100, 210, 70, "0xFF33ffff");
			add(mainPngButton);
			add(mainXmlButton);
	...
	function writeCharacters() {
		...
		arrayOfStrings.push(flshText.text);
		...
		if (parsedWeekJson.version == 1 || parsedWeekJson.version == null) {
			...
			var coolObject:StoryMenuState.WeekInfo = {animation: coolSongArray[0], name: nameText.text, bf: bfText.text, gf: gfText.text, dad: dadText.text, songs: coolSongArray.slice(1), flshColor: arrayOfStrings.slice(1)};
