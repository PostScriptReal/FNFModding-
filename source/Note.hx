...
class Note extends DynamicSprite
{
	...
	public var rawNoteData:Int = 0; // KADE LUA FEATURE ONLY
	#end
	...
	public var beat:Float = 0; // KADE LUA FEATURE ONLY
	...
	#if FEATURE_LUAMODCHART
	public var modAngle:Float = 0; // KADE LUA ONLY FEATURE
	public var localAngle:Float = 0; // KADE LUA ONLY FEATURE
	#end
	...
	// haha funny hd mod
	public var dangerNote:Bool = false;
	// tabi mod
	...
	public var colourSwap:ColourSwap;
	...
	#if FEATURE_LUAMODCHART
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?customImage:Null<BitmapData>, ?customXml:Null<String>, ?customEnds:Null<BitmapData>, ?LiftNote:Bool=false, ?animSuffix:String, ?numSuffix:Int, ?bet:Float = 0)
	#else
	...
	#end
	{
		...
		{	
			...
			if (dangerNote)
			{
				animation.addByPrefix('greenScroll', 'green ohno${animSuffix}');
				animation.addByPrefix('redScroll', 'red ohno${animSuffix}');
				animation.addByPrefix('blueScroll', 'blue ohno${animSuffix}');
				animation.addByPrefix('purpleScroll', 'purple ohno${animSuffix}');
			}
		...
		if (noteData > -1 && !mineNote && !nukeNote && curUiType.noteShaderAffected) {
			colourSwap = new ColourSwap();
			var noteDat:Float = noteData % NOTE_AMOUNT;
			if (noteDat <= 4) {
				// All good!
			} else {
				noteDat = 4;
			}
			var noteNum:Int = Std.int(noteDat);
			var shaderJSON:Dynamic = CoolUtil.parseJson(FNFAssets.getText('assets/data/notes.json'));
			var parsedShader:NoteOptionsState.HSBJson = Reflect.field(shaderJSON, coolShaderArray[noteNum]);
