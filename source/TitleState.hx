...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		FlxG.save.close();
		#if USE_NEW_SAVE
		FlxG.save.bind("save"+preferredSave, 'Popularsonic');
		#else
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if (cmenu == 'default') {
				...
				if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/music&sounds/freakyMenu.ogg')) {
					FlxG.sound.playMusic('assets/images/custom_menus/' + cmenu + '/music&sounds/freakyMenu' + TitleState.soundExt, 0);
				} else {
					FlxG.sound.playMusic('assets/music/freakyMenu' + TitleState.soundExt, 0);
				}
			}	
				//Because of menu-packs, this is now deprecated.
		...
		if (cmenu == 'default') {
			...
			if (OptionsHandler.options.antialiasing != Everything) {
				titleBg.antialiasing = false;
			}
		} else {
			titleBg = new FlxSprite();
			titleBg.frames = FlxAtlasFrames.fromSparrow('assets/images/custom_menus/' + cmenu + '/gettin_freaky/titleBG.png', 'assets/images/custom_menus/' + cmenu + '/gettin_freaky/titleBG.xml');
			titleBg.antialiasing = !bgTitle.isPixel;
			titleBg.animation.addByPrefix('thefunny', "bg", 24, true);
			titleBg.animation.play('thefunny');
			if (OptionsHandler.options.antialiasing != Everything) {
				titleBg.antialiasing = false;
			}
		...
		if (cmenu == 'default') {
			...
			logoBl = new FlxSprite(logoTitle.curX, logoTitle.curY);
			logoBl.frames = FlxAtlasFrames.fromSparrow('assets/images/custom_menus/' + cmenu + '/gettin_freaky/logoBumpin.png', 'assets/images/custom_menus/' + cmenu + '/gettin_freaky/logoBumpin.xml');
			logoBl.antialiasing = !logoTitle.isPixel;
		...
		if (cmenu == 'default') {
			...
			if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/gettin_freaky/gfDanceTitle.png')) {
				if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/gettin_freaky/gfDanceTitle.xml')) {
					gfDance = new FlxSprite(gfTitle.curX, gfTitle.curY);
					gfDance.frames = FlxAtlasFrames.fromSparrow('assets/images/custom_menus/' + cmenu + '/gettin_freaky/gfDanceTitle.png', 'assets/images/custom_menus/' + cmenu + '/gettin_freaky/gfDanceTitle.xml');
					if (OptionsHandler.options.antialiasing != Everything) {
						gfDance.antialiasing = false;
					}
				}
		...
		if (cmenu == 'default') {
			...
			if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/gettin_freaky/titleEnter.png')) {
				if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/gettin_freaky/titleEnter.xml')) {
					titleText = new FlxSprite(100, FlxG.height * 0.8);
					titleText.frames = FlxAtlasFrames.fromSparrow('assets/images/custom_menus/' + cmenu + '/gettin_freaky/titleEnter.png', 'assets/images/custom_menus/' + cmenu + '/gettin_freaky/titleEnter.xml');
					titleText.animation.addByPrefix('idle', "Press Enter to Begin", 24);
					titleText.animation.addByPrefix('press', "ENTER PRESSED", 24);
					titleText.antialiasing = true;
					titleText.animation.play('idle');
		...
		if (cmenu == 'default') {
			...
			if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/gettin_freaky/newgrounds_logo.png')) {
				ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic('assets/images/custom_menus/' + cmenu + '/gettin_freaky/newgrounds_logo.png');
				add(ngSpr);
				ngSpr.visible = false;
				ngSpr.setGraphicSize(Std.int(ngSpr.width * 0.8));
				ngSpr.updateHitbox();
	...
	function getIntroTextShit():Array<Array<String>>
	{
		...
		if (cmenu == 'default') {
			...
			if (FNFAssets.exists('assets/images/custom_menus/' + cmenu + '/gettin_freaky/introText.txt')) {
				fullText = Assets.getText('assets/images/custom_menus/' + cmenu + '/gettin_freaky/introText.txt');
			} else {
				fullText = Assets.getText('assets/data/introText.txt');
			}
		}
	...
	override function beatHit()
	{
		...
		if (curBeat < 9) {
			switch (curBeat)
			{
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['m++ engine', 'created by']);
				case 7:
