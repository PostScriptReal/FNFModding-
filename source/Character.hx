...
/*enum abstract Experiments(Int) from Int to Int {
	var NONE = 0;
	var SICK = 1;
}*/
typedef TCharacterRefJson = {
	...
	var customHealthColour:Bool;
	var healthColour:String;
}
class Character extends FlxSprite
{
	...
	// public var experiments:Experiments = NONE;
	// like bf, is playable
	public var likeBf:Bool = false;
	public var isDie:Bool = false;
	public var isPixel:Bool = false;
	public var customHealthColourr:Bool = false;
	private var interp:Interp;
	// public var sickCharOpt:Int = 0;
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		var healthColourChk:TCharacterRefJson = Reflect.field(charJson, curCharacter);
		customHealthColourr = healthColourChk.customHealthColour;
		trace(customHealthColourr);
		if (customHealthColourr && healthColourChk.healthColour == null) {
			bfColor = FlxColor.fromString(healthColourChk.colors[0]);
			enemyColor = FlxColor.fromString(healthColourChk.colors[0]);
			opponentColor = FlxColor.fromString(healthColourChk.colors[0]);
			playerColor = FlxColor.fromString(healthColourChk.colors[0]);
		} else if (customHealthColourr && healthColourChk.healthColour != null) {
			bfColor = FlxColor.fromString(healthColourChk.healthColour);
			enemyColor = FlxColor.fromString(healthColourChk.healthColour);
			opponentColor = FlxColor.fromString(healthColourChk.healthColour);
			playerColor = FlxColor.fromString(healthColourChk.healthColour);
		}
	...
	public function sing(direction:Int, ?miss:Bool=false, ?alt:Int=0/*, ?daRating:String = "sick"*/) {
		var directName:String = "";
		var missName:String = "";
		/*sickCharOpt = interp.variables.get("SICK");
		var sicksuffix:String = "";*/
		/*if (daRating == "sick") {
			sicksuffix = "-SICK";
		} else {
			sicksuffix = "";
		...
		switch (direction) {
			case 0:
				directName = "singLEFT"/* + sicksuffix*/;
			case 1:
				directName = "singDOWN"/* + sicksuffix*/;
			case 2:
				directName = "singUP"/* + sicksuffix*/;
			case 3:
				directName = "singRIGHT"/* + sicksuffix*/;
		}
	...
	public static function getAnimInterp(char:String):Interp {
		...
		var rawXml:String = "";
		rawPic = FNFAssets.getBitmapData('assets/images/custom_chars/gf-tankmen/char.png');
		rawXml = FNFAssets.getText('assets/images/custom_chars/gf-tankmen/char.xml');
		var gfTex = FlxAtlasFrames.fromSparrow(rawPic, rawXml);
		...
		interp.variables.set("SICK", SICK);*/
		interp.variables.set("Level_NotAHoe", Level_NotAHoe);
		...
		interp.variables.set("dynaCam", OptionsHandler.options.weakCamera);
		interp.variables.set("colors", [FlxColor.CYAN]);
		interp.variables.set("gfTex", gfTex);
