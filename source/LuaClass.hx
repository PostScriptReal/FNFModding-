...
{ // again, stolen from andromeda but improved a lot for better thinking interoperability (I made that up)
	...
	public var sprite:KadeArrow;

	public function new(connectedSprite:KadeArrow, name:String)
	{
		super();
	...
	{
		for (i in 0...PlayState.kadeStrumLineNotes.length)
		{
			if (index == i)
			{
				return PlayState.kadeStrumLineNotes.members[i];
			}
		}
		return null;
	}
...
{ // again, stolen from andromeda but improved a lot for better thinking interoperability (I made that up)
	...
	private static function changeCharacter(l:StatePointer):Int
	{
		...
		PlayState.kadeDad = new Character(x, y, newName, char.isPlayer);

		property.char = PlayState.kadeDad;

		PlayState.instance.add(PlayState.kadeDad);
...
{ // again, stolen from andromeda but improved a lot for better thinking interoperability (I made that up)
	...
	public function new()
	{
		...
		properties = [
			...
			"accuracy" => {
				defaultValue: PlayState.instance.kadeAccuracy,
				getter: function(l:State, data:Any):Int
				{
					Lua.pushnumber(l, PlayState.instance.kadeAccuracy);
					return 1;
				},
				setter: SetNumProperty
			},
	...
	private static function changeStage(l:StatePointer):Int
	{
		...
		for (i in PlayState.LuaStage.toAdd)
		{
		...
		PlayState.makeHaxeState("stages", "assets/images/custom_stages/" + stageName + "/", "../"+Reflect.field(stageJson, stageName));
	...
	private var changeStageC:cpp.Callable<StatePointer->Int> = cpp.Callable.fromStaticFunction(changeStage);
