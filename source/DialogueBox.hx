...
class DialogueBox extends FlxSpriteGroup
{
	...
	{	
		...
		skipText = new FlxText(10, FlxG.height, 0, "Press TAB to skip dialog", 150);
		if (_dialogue.isPixel)
		{
			skipText.setFormat("assets/fonts/pixel.otf", 20, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		} else {
			skipText.setFormat("assets/fonts/funkin.otf", 20, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		}
		add(skipText);
	...
	override function update(elapsed:Float)
	{
		...
		} else if (FlxG.keys.justPressed.ANY && !FlxG.keys.justPressed.TAB && dialogueStarted == true)
		{
			remove(dialogue);
	...
	function justGetThisOverWith():Void {
		isEnding = true;
		skipText.visible = false;
		remove(skipText);
		...
		new FlxTimer().start(fadeOutTime / 2, function(tmr:FlxTimer)
		{
			box.alpha -= 1 / fadeOutLoop;
			bgFade.alpha -= 1 / fadeOutLoop * 0.7;
			portrait.visible = false;
		...
		new FlxTimer().start(fadeOutTime * (fadeOutLoop + 1) / 2, function(tmr:FlxTimer)
		{
			finishThing();
			kill();
		});
