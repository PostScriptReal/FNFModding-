...
class FreeplayState extends MusicBeatState
{
	...
	var replaceSongListt:Bool = false;
	var getRSL:String;
	var modUses:String;
	...
	override function create()
	{
		...
		var getData = CoolUtil.coolTextFile('mods/mod/modData.moddat');
		getRSL = getData[2].toString();
		modUses = getData[1].toString();
		if (getRSL == 'true') {
			replaceSongListt = true;
		} else if (getRSL == 'false') {
			replaceSongListt = false;
		} else if (getRSL != 'true' && getRSL != 'false') {
			throw "The mod's local save data has been corrupted!";
		}
		...
		if (!FlxG.sound.music.playing)
		{
			if (cmenu == 'default') {
				...
				FlxG.sound.playMusic('assets/images/custom_menus/' + cmenu + '/music&sounds/freakyMenu' + TitleState.soundExt);
			}
		...
		if (cmenu == 'default') {
			...
			bg.loadGraphic('assets/images/custom_menus/' + cmenu + '/main_menus/menuDesat.png');
		}
		...
		scoreBG = new FlxSprite(scoreText.x - 6, 0).makeGraphic(Std.int(FlxG.width * 0.4), 66, 0xFF000000);
		scoreBG.alpha = 0.6;
		if (OptionsHandler.options.antialiasing != Everything) {
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK)
		{
			...
			if (soundTest)
				...
				if (modUses != 'none' && replaceSongListt) {
					epicCategoryJs = CoolUtil.parseJson(FNFAssets.getJson('mods/'+ modUses +'/data/freeplaySongJson'));
				}
		...
		if (accepted)
		{
			...
			if (soundTest)
			{
				...
				if (soundTestSong.needsVoices)
				{
					if (modUses != 'none' && replaceSongListt) {
						if (OptionsHandler.options.stressTankmen
							&& FNFAssets.exists("mods/"+ modUses +"/music/" + soundTestSong.song + "Shit" + suffix + "_Voices" + TitleState.soundExt))
							shit = "Shit";
					} else {
						if (OptionsHandler.options.stressTankmen
					...
					var vocalSound;
					if (modUses != 'none' && replaceSongListt) {
						vocalSound = FNFAssets.getSound("mods/"+ modUses +"/music/" + soundTestSong.song + shit + suffix + "_Voices" + TitleState.soundExt);
					} else {
						vocalSound = FNFAssets.getSound("assets/music/" + soundTestSong.song + shit + suffix + "_Voices" + TitleState.soundExt);
					}
				...
				if (modUses != 'none' && replaceSongListt) {
					if (OptionsHandler.options.stressTankmen
						&& FNFAssets.exists("mods/"+ modUses +"/music/" + soundTestSong.song + "Shit" + suffix + "_Inst" + TitleState.soundExt))
						shit = "Shit";
					else
						shit = "";
				} else {
				...
				if (modUses != 'none' && replaceSongListt) {
					FlxG.sound.playMusic(FNFAssets.getSound("mods/"+ modUses +"/music/" + soundTestSong.song + suffix + "_Inst" + TitleState.soundExt), curDifficulty == 2 ? 0 : 1);
				} else {
			...
			else
			{
				...
				trace(poop);
				if (modUses != 'none' && replaceSongListt) {
					if (FNFAssets.exists('mods/'+ modUses +'/data/' + songs[curSelected].songName.toLowerCase() + '/' + poop.toLowerCase() + '.json'))
					{
	...
	{
		if (modUses != 'none' && replaceSongListt) {
			FlxG.sound.play('mods/'+ modUses +'/sounds/custom_menu_sounds/'+CoolUtil.parseJson(FNFAssets.getText("mods/"+ modUses +"/sounds/custom_menu_sounds/custom_menu_sounds.json")).customMenuScroll+'/scrollMenu' + TitleState.soundExt, 0.4);
		} else {
