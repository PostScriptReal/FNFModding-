...
import OptionsHandler.Antialiasing;
import OptionsHandler.Resolution;
...
class SaveDataState extends MusicBeatState
{
	...
	public static var graphics:Bool = false;
	public static var preferences:Bool = false;
	public static var gameplay:Bool = false;
	public static var modding:Bool = false;
	var optionMask:Mask<FullOptions>;
	/*var ifGraphics:Mask<FullOptions>;
	var ifPreferences:Mask<FullOptions>;
	var ifGameplay:Mask<FullOptions>;
	var ifModding:Mask<FullOptions>;*/
	...
	override function create()
	{
		...
		// This old category system is about to get a glow-up
		// It's soooooooooooo unintuitive
		/*ifGraphics = CoolUtil.parseJson(FNFAssets.getJson('assets/data/graphics'));
		ifPreferences = CoolUtil.parseJson(FNFAssets.getJson('assets/data/preferences'));
		ifGameplay = CoolUtil.parseJson(FNFAssets.getJson('assets/data/gameplay'));
		ifModding = CoolUtil.parseJson(FNFAssets.getJson('assets/data/modding'));*/
		...
		if (OptionsHandler.options.antialiasing != Everything) {
			...
			optionList = [
							{name: "Anti-epilepsy measures", intName: "nomoreepilepsy", value: false, desc: "When enabled, Anti-epilepsy measures will be enabled, altering content that's harmful to the player.", cat: "Preferences"},
							{name: "Naughtiness", intName: "hahanaughty", value: false, desc: "Basically the naughtiness option from week 7, except it actually works this time.", cat: "Preferences"},
							{name: "Always Show Cutscenes", intName: "alwaysDoCutscenes", value: false, desc: "Force show cutscenes, even in freeplay", cat: "Preferences"},
							{name: "Lyrics", intName: "lyrics", value: false, desc: "Enable or disable lyrics shown during gameplay in songs that contain them.", cat: "Preferences"},  
							{name: "Skip Modifier Menu", value: false, intName: "skipModifierMenu", desc: "Skip the modifier menu", cat: "Preferences"}, 
							{name: "Skip Victory Screen", value: false, intName : "skipVictoryScreen", desc: "Skip the victory screen at the end of songs.", cat: "Preferences"},
							...
							{name: "Downscroll", value: false, intName: "downscroll", desc: "Put da arrows on the bottom and have em scroll down", cat: "Gameplay"},
							{name: "Middlescroll", value: false, intName: "middlescroll", desc: "Enemy strums go bye, bye, and bam! Your notes are centered.", cat: "Gameplay"},
							{name: "Don't mute on miss", intName: "dontMuteMiss", value: false, desc: "When missing notes, don't mute vocals", cat: "Preferences"},
							{name: "Judge", value: false, intName: "judge", desc: "The Judge to use.", amount: cast Judge.Jury.Classic, defAmount: cast Judge.Jury.Classic, max: 10, cat: "Gameplay"},
							{name: "Ghost Tapping", value: false, intName: "useCustomInput", desc: "Whether to allow spamming", cat: "Gameplay"},
							{name: "Play Miss Animation", value: false, intName: "playMissAnim", desc: "Whether to play miss animations when getting a miss, shit or wayoff.", cat: "Gameplay"},
							{name: "Dynamic Camera Movement", value: false, intName: "weakCamera", desc: "Whether to have the camera automatically adjust to a character's pose or not.", cat: "Gameplay"},
							...
							{name: "Show Song Position", value: false, intName: "showSongPos", desc: "Whether to show the song bar.", cat: "Preferences"},
							{name: "Style", value: false, intName: "style", desc: "Whether to use fancy style or default to base game.", cat: "Preferences"},
							{
								name: "Ignore Unlocks",
								value: false,
							...
							{name: "Overwrite Judgement", value: false, intName: "preferJudgement", desc: "What judgement to display other than default, if any.", amount: 0, defAmount: 0, min: -1, max: 2, cat: "Preferences"},
							{name: "Note splashes", value: false, intName: "togglesplash", desc: "Toggle the note splashes that appear when you get a sick! rating.", cat: "Preferences"},
							{name: "Custom Menus", value: false, intName: "menu", desc: "Custom Menus! This option selects whatever menu you prefer, if custom menus are provided.", defAmount: 0, amount: 0, max: CoolUtil.coolTextFile('assets/data/menupacks.txt').length - 1, cat: "Preferences"},
							{name: "Antialiasing", value: false, intName: "antialiasing", desc: "Toggles Antialiasing, turning it off can improve performance, but will make stuff look jaggier.", defAmount: 0, amount: 0, min: 0, max: 4, cat: "Graphics"},
							{name: "Optimisation", value: false, intName: "toaster", desc: "The big performance! Removes characters and stages to improve performance.", cat: "Graphics"},
							{name: "VCR Effect", value: false, intName: "vhs", desc: "Removes the VCR effect from week 6 if disabled.", cat: "Graphics"},
							// {name: "Upscale Internal Resolution", value: false, intName: "screenresolution", desc: "Change the game's internal resolution, default is 1280x720. Sprites will be scaled to fit new resolution unless hd sprites are provided.", defAmount: 0, amount: 0, min: 0, max: 1},
							{name: "Emulate Osu Lifts", value: false, intName: "emuOsuLifts", desc: "Whether to add lift notes at the end of sustains to force releasing buttons.", cat: "Gameplay"},
							{name: "Show Combo Breaks", value: false, intName:"showComboBreaks", desc: "Whether to display any combo breaks by flashing the screen.", cat: "Preferences"},
							{name: "Funny Songs", value: false, intName: "stressTankmen", desc: "funny songs", cat: "Gameplay"},
							//{name: "SiIvagunner Gags", value: false, intName: "siivagunnermode", desc: "When checked, SiIvagunner rips (or any supported songs) will insta-kill you before reaching the end (HIGH-SCORES CAN'T BE SAVED WITH THIS OPTION TURNED ON).", cat: "Gameplay"},
							{name: "Use Kade Health", value: false, intName: "useKadeHealth", desc: "Use kade engines health numbers when healing and dealing damage", cat: "Gameplay"},
							{name: "Use Miss Stun", value: false, intName: "useMissStun", desc: "Prevent hitting notes for a short time after missing.", cat: "Gameplay"},
							// {name: "Resume Behaviour", value: false, intName: "funnyCloneHero", desc: "If enabled, resuming will rewind the song when notes are nearby. (Just like Clone Hero!) ", cat: "Gameplay"},
							{name: "Don't Use Vile Rating", value: false, intName: "ignoreVile", desc: "Don't use the \"Vile\" rating", cat: "Gameplay"},
							{name: "Offset", value: false, intName: "offset", desc: "How much to offset notes when playing. Can fix some latency issues! Hold Control to scroll faster.", amount: 0, defAmount: 0, max: 1000, min: -1000, precision: 0.1, cat: "Gameplay"},
							{name: "Accuracy Mode", value: false, intName: "accuracyMode", desc: "How accuracy is calculated. Complex = uses ms timing, Simple = uses rating only", amount: 0, defAmount: 0, min: -1, max: 2, cat: "Gameplay"},
							//{name: "Credits", value: false, intName:'credits', desc: "Show the credits!", ignore: true},
							{name: "Sound Test...", value: false, intName: 'soundtest', desc: "Listen to the soundtrack", ignore: true, cat: "Modding"},
							{name: "Controls...", value: false, intName:'controls', desc:"Edit bindings!", ignore: true, cat: "Gameplay"},
							{name: "Hit Sounds", value: false, intName:"hitSounds", desc: "Play a sound when hitting a note", defAmount: 0, amount: 0, max: CoolUtil.coolTextFile('assets/data/hitSounds.txt').length - 1, cat: "Preferences"},
							{name: "Fps Cap", value: false, intName: "fpsCap", desc: "What should the max fps be.", amount: 60, defAmount: 60, max: 240, min: 20, precision: 10, cat: "Preferences"},
							{name: "Allow Story Mode", value: false, intName:"allowStoryMode", desc: "Show story mode from the main menu.", cat: "Preferences"},
							{name: "Allow Freeplay", value: false, intName:"allowFreeplay", desc: "Show freeplay from the main menu.", cat: "Preferences"},
							{name: "Allow Donate Button", value: false, intName:"allowDonate", desc: "Show the donate button from the main menu.", cat: "Preferences"},
							{name: "Funny BF", value: false, intName:"me", desc: "I see you digging around my source code mate! Uncomment this for a treat!", cat: "Modding"},
							#if sys
							{name: "Toggle Title Background", value: true, intName:'titleToggle', desc:"Turn on/off the title screen background.", ignore: true, cat: "Preferences"},
							{name:"New Character...", value: false, intName:'newchar', desc: "Make a new character!", ignore: true, cat: "Modding"},
							{name:"New Stage...", value:false, intName:'newstage', desc: "Make a new stage!", ignore: true, cat: "Modding"},
							{name: "New Song...", value: false, intName:'newsong', desc: "Make a new song!", ignore: true, cat: "Modding"},
							{name: "New Week...", value: false, intName: 'newweek', desc: "Make a new week!", ignore: true, cat: "Modding"},
							{name: "Sort...", value: false, intName: 'sort', desc: "Sort some of your current songs/weeks!", ignore : true, cat: "Modding"}
							#end
						];
		var kidsToKill:Array<TOption> = [];
		var kidsToHide:Array<TOption> = [];
		for (option in optionList) {
			if (Reflect.field(optionMask, option.intName) != null && !Reflect.field(optionMask, option.intName))
				kidsToKill.push(option);
		}
		if (graphics) {
			for (hide in optionList) {
				if (hide.cat != 'Graphics' && graphics)
					kidsToHide.push(hide);
			}
		}
		if (preferences) {
			for (hide in optionList) {
				if (hide.cat != 'Preferences' && preferences)
					kidsToHide.push(hide);
			}
		}
		if (gameplay) {
			for (hide in optionList) {
				if (hide.cat != 'Gameplay' && gameplay)
					kidsToHide.push(hide);
			}
		}
		if (modding) {
			for (hide in optionList) {
				if (hide.cat != 'Modding' && modding)
					kidsToHide.push(hide);
			}
		}
		...
		*/
		...
		if (graphics || preferences || gameplay || modding) {
			for (kid in kidsToHide) {
				optionList.remove(kid);
			}
		}
		trace("hmmm");
		...
		for (j in 0...optionList.length) {
			...
			/*if (graphics) {
				if (Reflect.field(ifGraphics, optionList[j].intName) != null && !Reflect.field(ifGraphics, optionList[j].intName))
				{
					continue;
				}
			}*/
			...
			if (cmenu == 'default') {
				...
				if (OptionsHandler.options.antialiasing != Everything) {
					coolCheckmark.antialiasing = false;
				}
			} else {
		...
		/*var cmenucoolstringpartwo:String = '';
		if (cmenu == 'default') {
			menuBG.color = 0xFF7194fc;
			cmenucoolstringpartwo = cmenucoolstring;
			cmenucolour = FlxColor.fromString(cmenucoolstringpartwo);
			menuBG.color = cmenucolour;
		}*/
		menuBG.color = 0xFF7194fc;
		menuBG.setGraphicSize(Std.int(menuBG.width * 1.1));
		menuBG.updateHitbox();
	...
	override function update(elapsed:Float) {
		...
		if (controls.BACK) {
			if (!saves.members[curSelected].beingSelected) {
				...
				/*if (OptionsHandler.options.screenresolution == R1080p) {
		            internalUpscale(1920, 1080);
		        }*/
			...
			if (saves.members[curSelected].beingSelected) {
				if (!saves.members[curSelected].askingToConfirm) {
					if (saves.members[curSelected].selectingLoad) {
						...
						FlxG.save.bind(saveName, "Popularsonic");
					...
					FlxG.save.data.songScores = ["tutorial" => 0];
					FlxG.save.bind(oldSave, "Popularsonic");
					Highscore.load();
				}
	...
	function changeAmount(increase:Bool=false) {
		...
		if (optionList[optionsSelected].intName == "hitSounds") {
			var menuList = CoolUtil.coolTextFile('assets/data/hitSounds.txt');
			numberDisplays[optionsSelected].text = menuList[Std.int(optionList[optionsSelected].amount)];
		}
		...
		if (optionList[optionsSelected].intName == "antialiasing") {
			switch (cast (Std.int(optionList[optionsSelected].amount) : OptionsHandler.Antialiasing)) {
				case Everything: 
					numberDisplays[optionsSelected].text = "Everything";
				case CharactersStages:
					numberDisplays[optionsSelected].text = "Characters & Stages";
				case Characters:
					numberDisplays[optionsSelected].text = "Characters";
				case None:
					numberDisplays[optionsSelected].text = "None";
			}
		}
		if (optionList[optionsSelected].intName == "screenresolution") {
			switch (cast (Std.int(optionList[optionsSelected].amount) : OptionsHandler.Resolution)) {
				case Default: 
					numberDisplays[optionsSelected].text = "Default";
				case R1080p:
					numberDisplays[optionsSelected].text = "1.5x (1080p)";
			}
		}
	}
	...
	public function internalUpscale(widthh:Int, heightt:Int)
	{
		Lib.application.window.resize(widthh, heightt);
	}
}
