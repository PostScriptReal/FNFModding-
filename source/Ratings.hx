...
class Ratings
{
	...
	public static function CalculateRating(noteDiff:Float, ?customSafeZone:Float):String // Generate a judgement through some timing shit
	{
		...
		if (noteDiff > Judge.badJudge) // way early
			...
			return "miss";
		/*else if (PlayState.waitHow)
			PlayState.waitHow = false;
			return "wait how did you do that";*/
		return "sick";
	}
	public static function CalculateFullCombo(level:FCLevel):Bool {
		return switch (level) {
			/*case How:
				(PlayState.misses == 0 && PlayState.bads == 0 && PlayState.shits == 0 && PlayState.goods == 0 && PlayState.sicks == 0);*/
			case Sick:
