...
class ModifierState extends MusicBeatState
{
	...
	var startInChart:Bool = false;
	/*var secretInt:Int;
	public static var oooo:Bool;*/
	/*var tab_group_char:FlxUI;
	var secretTextField:FlxInputText;
	...
	override function create()
	{
		...
		if (cmenu == 'default') {
			...
			menuBG = new FlxSprite().loadGraphic('assets/images/custom_menus/' + cmenu + '/main_menus/menuBGMagenta.png');
		}
		...
		if (OptionsHandler.options.me) {
			FlxG.mouse.visible = true;
			tab_group_char = new FlxUI(null, UI_box);
			tab_group_char.name = "Secret";
			secretTextField = new FlxUIInputText(FlxG.width - 10, FlxG.height - 10, secretString, 8);
		}*/
		add(menuBG);
		add(grpAlphabet);
		add(multiTxt);
		add(description);
		// tab_group_char.add(secretTextField);
		calculateMultiplier();
	...
	override function update(elapsed:Float) {
		...
		/*if (OptionsHandler.options.me) {
			secretString = secretTextField.text;
		}
		if (!secretTextField.hasFocus) {*/
		if (controls.BACK) {
		...
		if (controls.ACCEPT)
			toggleSelection();
		if (FlxG.keys.justPressed.SEVEN && !startInChart) {
			startInChart = true;
		} else if (FlxG.keys.justPressed.SEVEN && startInChart) {
			startInChart = false;
		}
	}
